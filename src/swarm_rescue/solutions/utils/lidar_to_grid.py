...
class OccupancyGrid(Grid):
    ...
    def update_grid(self, pose: Pose):
        ...
        self.zoomed_grid = cv2.resize(self.zoomed_grid, new_zoomed_size, interpolation=cv2.INTER_AREA)
    ...
    def Astar_to_grid_index(self, i, j):
        return i, len(self.grid[i]) - j


# The following represents an exemple of Grid usage

class MapMappingExemple(MapAbstract):
...
class MyDroneMappingExemple(DroneAbstract):
...
def main():
    my_map = MapMappingExemple()
    playground = my_map.construct_playground(drone_type=MyDroneMappingExemple)
